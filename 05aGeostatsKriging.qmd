...
**Simple kriging (SK)** assumes the mean of the process is known rather than estimated from the data. In practice that assumption is almost never defensible, so SK sees limited use outside of simulation studies and textbooks. If you are curious how the math differs from OK, the [Kriging Variance By Hand aside](05xAsideGeostatsKrigingVariance.html) works through both side by side. In `gstat` you get SK by passing `beta = mean(dat$z)` to `krige`.
