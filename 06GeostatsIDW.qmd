...
2. `gridCA.rds` is data frame with two columns `X` and `Y` that represent an empty grid of cells that we will interpolate the precip values into like we did with `meuse.grid2` above. The spacing is 10x10 km for each point.
