...
2. Plot a correlogram of the OLS residuals with a bootstrap confidence envelope. This is the go/no-go decision: if the correlogram stays inside the envelope, the residuals are consistent with independence and you can stop here -- OLS is fine. Note that it's OK for $y$ or $x$ to be autocorrelated; what matters is the residuals.

3. If the correlogram pops above the envelope at short distances, fit a variogram to the residuals. The correlogram tells you that autocorrelation exists; the variogram tells you what its structure looks like -- the range, the sill, the nugget. You need that to specify the correlation structure for GLS.

4. Fit a GLS model with a spatial correlation structure based on the variogram model.
...
Generalized least squares (GLS) fixes this by explicitly accounting for the correlation structure of the errors. Instead of weighting each observation equally, GLS down-weights observations that are strongly correlated with their neighbors, because those observations are carrying less new information than they would if they were independent. In practice, this means we need to tell GLS what the correlation structure looks like, which is why we fit a variogram to the residuals first and pass that in. If the errors turn out to be independent, GLS simplifies and gives the same answer as OLS. OLS is a special case of GLS. The [OLS via Algebra and Matrices aside](10xAsideOLS-Algebra-Matrix.html) works through the matrix algebra if you want to see exactly how that works.
...
We are going to use $\epsilon$ as the error term in a model. I'll use the terms "errors" and "residuals" interchangeably throughout this book.
...
We fit a linear model of the form $y=\beta_0 + \beta_1x$ using GLS and got estimates for the intercept ($\hat\beta_0$) and slope ($\hat\beta_1$). We assumed that the error term was uncorrelated. Looking at summary, we can see the estimate of the slope ($\hat\beta_1$) is `r round(summary(glsNaive)$tTable[2,1],3)` and significantly different from zero. The important thing to wrap your head around here is that we just fit GLS with uncorrelated errors, meaning it's essentially identical to OLS. Compare the above to the more familiar `summary(lm(y~x,data=dat))`. The coefficients, standard errors, and t-statistics match. But the two summaries are laid out differently, and if you came up on `lm` the `gls` version will look a little foreign at first.
...
The header says "Generalized least squares fit by REML" where `lm` would say "Call." That tells you the estimation method, and it is the same REML from the callout above. Just under it you get AIC, BIC, and the log-likelihood printed by default, because a model fit by likelihood hands you those for free. With `lm` you'd call `AIC()` yourself.

Two things from the `lm` summary are missing on purpose. There is no $R^2$ and no overall F-statistic. $R^2$ is built on splitting the total sum of squares into an explained part and a residual part, and that split only behaves once you assume iid errors. Weight the observations by a covariance structure and the tidy decomposition falls apart, so `nlme` declines to print a number that would mislead you. If you go looking for $R^2$ and can't find it, that's why. Use AIC for comparison instead.

A few smaller cosmetic shifts. The coefficient columns are spelled `Value`, `Std.Error`, `t-value`, and `p-value`, and there are no significance stars. The residual five-number summary is labeled `Standardized residuals`, since these are normalized by the fitted covariance. And `gls` prints a `Correlation:` block by default that `lm` hides unless asked. Watch out: that block is the correlation among the coefficient estimates, not the spatial correlation of the residuals. Different thing.

When we add a correlation structure in a moment, the summary will grow one more block reporting the estimated range and nugget. That block is the whole reason we are here, and the naive model above doesn't have it.
...
We need to determine an appropriate theoretical model for the spatial structure of the residuals and pass that to `gls`. Before we do, it is worth thinking about what happens under the hood. Fair warning: the next paragraph uses matrix notation. If that is unfamiliar or rusty, the [OLS via Algebra and Matrices aside](10xAsideOLS-Algebra-Matrix.html) covers the basics. The short version is that matrix notation is just a compact way of writing the same equations you already know for all $n$ observations at once. You do not need to follow every symbol to understand what GLS is doing, but it helps to at least recognize the shape of the argument. GLS builds a variance-covariance matrix $\boldsymbol{\Sigma}$ for the residuals. Think of $\boldsymbol{\Sigma}$ as an $n \times n$ table where the diagonal entries are the variances of each observation and the off-diagonal entries $\boldsymbol{\Sigma}_{ij}$ record the covariance between observations $i$ and $j$ -- a function of the distance between locations $i$ and $j$, with the functional form coming from the correlation structure you specify. Then it estimates both the regression coefficients and the correlation parameters (the range, nugget, and partial sill) together by maximizing the likelihood. In matrix notation, the estimator is $\hat{\boldsymbol{\beta}} = (\mathbf{X}^T \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^T \boldsymbol{\Sigma}^{-1} \mathbf{y}$, where $\mathbf{X}$ is the design matrix (one row per observation, one column per predictor, plus a column of ones for the intercept) and $\mathbf{y}$ is the vector of response values. You can compare this to the OLS version $(\mathbf{X}^T \mathbf{X})^{-1} \mathbf{X}^T \mathbf{y}$: OLS weights every observation equally, GLS weights by the inverse of the covariance structure. The default method in `gls` is REML, which gives better estimates of the variance parameters than plain ML, at the cost of not being able to compare models with different fixed effects via likelihood ratio tests. You generally want REML for estimation and ML if you need to compare fixed effects with `anova`.
...
ML maximizes the likelihood of the data given all the parameters at once -- including both the fixed effects ($\boldsymbol{\beta}$) and the variance parameters (range, sill, nugget). The problem is that this treats the fixed effects as known when estimating the variance, which causes the variance estimates to be systematically too small, in the same way that dividing by $n$ rather than $n-1$ when computing a sample variance gives a biased result.
...
Notice we didn't call `gls` from scratch. We used `update`, which is a generic (the [methods and generics aside](02xAsideMethodsAndGenerics.html) lists it among the common ones) that takes a fitted model, digs out the call that produced it, swaps in whatever you name, and re-fits. So `update(glsNaive, correlation = csSpatial)` reaches back to the original `gls(y ~ x, dat)`, bolts on the correlation argument, and runs it again. The result is the same as typing `gls(y ~ x, data = dat, correlation = csSpatial)` by hand. The advantage is that you don't retype the formula and the data, which is exactly where a slip would creep in and leave you comparing two models that differ by more than the one thing you meant to change.
...
This is interesting. The summary now carries the extra block we promised: a `Correlation Structure` section reporting the range and nugget that `gls` estimated for the spherical model. Those are the parameter values the variogram only suggested the shape of. We will interpret more below but note that the slope coefficient is different from `glsNaive` to `glsUpdated`. Let's look at the spatial structure of the residuals from `glsUpdated`.
...
GLS is OLS with one additional ingredient: a model for the error correlation structure. The slopes are still there, the interpretation is still the same, but the standard errors now reflect how the data are actually distributed in space rather than assuming independence that doesn't exist. That's the whole point.

In practice, the most common outcome is what you saw here: the coefficients don't move much, but the standard errors get corrected, the AIC improves, and you can make your inferential claims without the nagging worry that your p-values are artifacts of spatial structure. Sometimes the coefficients do shift, usually when a predictor covaries with the spatial autocorrelation in the residuals. When that happens, GLS is doing more than tidying up your errors. It's telling you something about the model itself.

The next chapter takes this further. GLS treats spatial autocorrelation as a nuisance in the residuals. SAR models ask whether the spatial structure might be in the response itself, not just the errors, and fit accordingly.

## Exercises

The file `birdDiv.csv` has data on bird diversity and plant community diversity for 64 one-hectare plots across the Colorado Plateau and Mogollon Rim region of northern Arizona. The coordinates are UTM Zone 12N, NAD83 (EPSG:26912), in meters.

A note on the diversity values: they are non-integer and range from 3.16 to 12.95 for birds and 3.70 to 8.14 for plants. These are not raw species counts -- they are likely a diversity index such as Shannon or Fisher's alpha, though the original source and exact units have been lost. Higher values indicate greater diversity in both cases, and the relative patterns across plots are interpretable even if the absolute values are not.

::: {.callout-warning}

## A Note on Data Provenance

This dataset is a good reminder to document your data sources carefully and immediately. The coordinates, spatial extent, plot size, and biological variables here are all internally consistent and ecologically plausible, but the original source has been lost, making it impossible to properly cite, verify the methods, or understand the diversity metric. This happens more easily than you might think: a dataset downloaded for a class exercise, renamed, reformatted, and re-used a few years later can quickly become an orphan with no traceable origin. Keep a README alongside any dataset you use, noting the source, date accessed, and any transformations you applied.

:::

Model bird diversity as a function of plant diversity, with the expectation that higher plant diversity is associated with higher bird diversity. Follow the four-step workflow from this chapter: OLS first, correlogram of residuals, variogram if needed, GLS with an appropriate correlation structure. Watch those residuals and interpret carefully -- does accounting for spatial structure change the inference?

## Further Reading

@dormann2007methods review methods for accounting for spatial structure in species distribution modeling. The intro and section 3 on GLS are the most directly relevant. @keitt2002accounting show with ecological case studies how spatial methods outperform standard regression when dependence is ignored -- worth a skim for the results even if you skip the methodological details. Tim Keitt is a smarty pants and it's worth reading anything he's written.
