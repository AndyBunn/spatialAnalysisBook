  ...
  for (i in 1:nrow(covar)) {
    for (j in 1:ncol(covar)) {
      if (dist[i, j] > range) {
        ...
        covar[i, j] <- sigma - (nugget + (sigma - nugget) *
...
est2.sk <- krige(z ~ 1, ~ x + y,
  data = dat, model = dat.vgram,
  newdata = data.frame(x = 5, y = 5), beta = mean(dat$z)
)
est2.ok <- krige(z ~ 1, ~ x + y,
  data = dat, model = dat.vgram,
...
res.df <- data.frame(
  ...
  est = c(
    round(est.sk, 3), round(est2.sk$var1.pred, 3),
    round(est.ok, 3), round(est2.ok$var1.pred, 3)
  ),
  var = c(
    round(var.sk, 3), round(est2.sk$var1.var, 3),
