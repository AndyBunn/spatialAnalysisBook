...
We need to determine an appropriate theoretical model for the spatial structure of the residuals. Once we have a model family, we can pass it to the `gls` function via a `corStruct` object. The families available in `gls` match the theoretical variogram models we used with kriging: spherical, exponential, Gaussian, and so on. So, let's fit a variogram to the residuals and see which model fits best.
